...
public class EffectManager : MonoBehaviour
{
    ...
    public GameObject Generate(string filePath, Vector3 appearPos)
    {
        // 생성된 캐시를 불러옴
    ...
    /// <returns></returns>
    public bool Remove(string filePath, GameObject cacheObject)
    {
        // 키 값이 존재하는지 확인
        if (!fileCaches.ContainsKey(filePath))
        {
        ...
        // 캐시 반환
        return inGameSceneManager.cacheManager.Restore(filePath, cacheObject);
    }
