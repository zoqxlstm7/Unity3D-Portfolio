...
public class AnimalManager : MonoBehaviour
{
    ...
    public Animal Generate(string filePath, Vector3 appearPos)
    {
        // 생성된 캐시를 불러옴
        ...
        if (go)
        {
            ...
            animal.FilePath = filePath;
            // 적군 레이어로 변경
            animal.gameObject.layer = LayerMask.NameToLayer("Enemy");
            animal.targetMask = 1 << LayerMask.NameToLayer("Player");
    ...
    /// <returns></returns>
    public bool Remove(string filePath, Animal cacheObject)
    {
        // 키 값이 존재하는지 확인
        if (!fileCaches.ContainsKey(filePath))
        {
