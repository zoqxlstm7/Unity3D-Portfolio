...
public class AnimalDeadUIManager : MonoBehaviour
{
    #region Variables
    // 캐시 데이터
    ...
    public AnimalDeadUI Generate(string filePath, Vector3 appearPos)
    {
        // 생성된 캐시를 불러옴
        ...
        {
            // 캐시 데이터 설정
            AnimalDeadUI deadUI = go.GetComponent<AnimalDeadUI>();
            deadUI.FilePath = filePath;
    ...
    /// <returns></returns>
    public bool Remove(string filePath, AnimalDeadUI cacheObject)
    {
        // 키 값이 존재하는지 확인
        if (!fileCaches.ContainsKey(filePath))
        {
